...

{
    ...
    public function addItemToPlayer($item_id,$amount,$steamid) {
        if(is_numeric($amount) && ($amount > 0)) {
            $item_id = $this->database->sanatize($item_id);
            $steamid = $this->database->sanatize($steamid);
            if($this->checkIfItemExistsInGame($item_id)) {
                $Online = $this->checkIfPlayerIsOnline($steamid);
                if ($Online == 1) {
                    $this->insertIntoRemote("market_give", "v4b1", "", "$steamid", "$item_id", "$amount", "");
                    $response['status'] = 1;
                    return $response;
                }
                elseif ($Online == 0) {
    ...
    public function giveUserMoney($steamid,$amount) {
        if(is_numeric($amount) && ($amount > 0)) {
            $steamid = $this->database->sanatize($steamid);
            $Online = $this->checkIfPlayerIsOnline();
            if($Online == 1) {
                $this->insertIntoRemote("market_money_give","v4b1","","$steamid",$amount,"","");
                $response['status'] = 1;
                return $response;
            }
            elseif($Online == 0) {
    ...
    public function takeUserMoney($steamid,$amount) {
        if(is_numeric($amount) && ($amount > 0)) {
            $steamid = $this->database->sanatize($steamid);
            $Online = $this->checkIfPlayerIsOnline();
            if($Online == 1) {
                $this->insertIntoRemote("market_money_take","v4b1","","$steamid",$amount,"","");
                $response['status'] = 1;
                return $response;
            }
            elseif($Online == 0) {
...
$web_to_server->addItemToPlayer("cw_m3super90", 1, "STEAM_0:0:81010017");
?>
